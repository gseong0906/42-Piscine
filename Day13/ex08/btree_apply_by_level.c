...
/*   Updated: 2019/07/26 15:47:48 by sjiseong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_func(t_rb_node **curr_lev, int lev, int len,
...
void	btree_apply_from_level1(t_rb_node **pre_lev, t_rb_node **curr_lev,
		int *arr,
		void (*applyf)(void *item, int current_level, int is_first_elem))
{
	int	i;
	...
	curr_lev = (t_rb_node**)malloc(sizeof(t_rb_node*) * arr[1]);
	i = 0;
	j = 0;
	while (i < arr[1] / 2)
	{
		...
		void (*applyf)(void *item, int current_level, int is_first_elem))
{
	...
	if (!root)
		return ;
	pre_lev = (t_rb_node**)malloc(sizeof(t_rb_node*));
	curr_lev = 0;
	pre_lev[0] = root;
